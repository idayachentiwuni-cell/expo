...
function RequestRow({ row }: { row: Row }) {
  ...
  const isError = isCompleted && (row.errorDescription != null || (status != null && (status < 200 || status >= 300)));
  const leftBorderColor = isError
    ? theme.icon.danger
    : isCompleted
    ? theme.icon.success
    : theme.border.default;
  const statusColor = statusCodeColor(theme, isCompleted, status, row.kind === 'completed' ? row.errorDescription : null);
    ...
    <View
      ...
      <View style={styles.rowHeader}>
        ...
        <View style={styles.rowHeaderRight}>
          {isCompleted && row.wasCached ? (
            <Text style={[styles.badge, styles.cachedBadge, { color: theme.text.info, borderColor: theme.border.info }]}>
              cached
            </Text>
          ) : null}
          {isCompleted && row.networkProtocol ? (
            <Text style={[styles.badge, { color: theme.text.secondary, borderColor: theme.border.default }]}>
              {row.networkProtocol}
            </Text>
          ) : null}
          <Text style={[styles.status, { color: statusColor }]}>
            {isCompleted ? (status != null ? String(status) : row.errorDescription ?? 'error') : '…'}
          </Text>
        </View>
      </View>
