...
class NetworkRequestInterceptor private constructor(
  ...
  override fun intercept(chain: Interceptor.Chain): Response {
    ...
    // Recursion guard: telemetry uploads carry this header so they aren't observed in turn.
...
internal fun buildSnapshot(
  ...
  val wasCached = response?.let { it.cacheResponse != null && it.networkResponse == null } ?: false

  val requestHeaderBytes = originalRequest.headers.byteCount()
  val responseHeaderBytes = response?.headers?.byteCount() ?: 0L
...
class NetworkRequestEventListener : EventListener() {
  ...
  companion object {
     ...
     * call when the call ends, the interceptor reads + removes when it builds the snapshot. The
     * map only ever holds completed-but-not-yet-snapshotted entries, which is bounded by the
     * tiny race window between event-listener completion and interceptor finalization.
     */
    private val inFlightTimings = ConcurrentHashMap<Call, Timings>()

    /** Removes and returns the timings for `call`, or `null` if none were recorded. */
    internal fun takeTimings(call: Call): Timings? = inFlightTimings.remove(call)
...
private fun Headers.byteCount(): Long {
  // OkHttp's `Headers.byteCount()` exists on newer versions; for portability across the OkHttp
  // pin shipped by RN, we compute it manually: "Name: Value\r\n" per header plus the trailing
  // "\r\n" that terminates the block.
  var total = 2L
  for (i in 0 until size) {
    total += name(i).length + 2 + value(i).length + 2
  }
  return total
}
